...
import { Plus, Pencil, Trash2, Search, Scissors, Mail, Phone } from 'lucide-react';
...
const ListaProfissionais = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.id) {
        await axios.put(`http://localhost:3000/profissionais/${formData.id}`, formData);
      } else {
        await axios.post('http://localhost:3000/profissionais', formData);
      }
      carregarProfissionais();
      setShowForm(false);
      setFormData({ nome: '', especialidade: '', email: '', fone: '' });
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Tem certeza que deseja excluir este profissional?')) {
      try {
        await axios.delete(`http://localhost:3000/profissionais/${id}`);
        carregarProfissionais();
      } catch (error) {
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-purple-800">Profissionais</h1>
        <button
          onClick={() => {
            setFormData({ nome: '', especialidade: '', email: '', fone: '' });
            setShowForm(true);
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {profissionaisFiltrados.map((profissional) => (
                <tr key={profissional.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">{profissional.nome}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{profissional.especialidade}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{profissional.email}</td>
                  <td className="px-6 py-4 whitespace-nowrap">{profissional.fone}</td>
